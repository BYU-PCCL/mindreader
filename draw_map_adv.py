...
def main():
	...
	#X1, Y1, X2, Y2 
	poly_map = polygons_to_segments(load_polygons_here(fn="./my_map_0.txt"))
	#print poly_map
	scale = 500
 ...
 # 	isovist = iso.Isovist( polygonSegments )
	...
	while True:
		mouseClick = Update()
		if mouseClick != None:
			if 90+100 > mouseClick[0] > 90 and 450+50 > mouseClick[1] > 450:
				pygame.draw.rect(screen, bright_green, (90,450,100,50))#pygame.Rect(left, top, width, height))#(150,450,100,50))
				#print curr_poly
				my_map.append(curr_poly)
				curr_poly= []
				print my_map

				file = open("my_intent_map_1.txt","w")
				for poly in my_map:
					poly_str = ""
					for i in xrange(len(poly)):
						poly_str += str(poly[i][0]*1000)
						poly_str += " "
						poly_str += str(poly[i][1]*1000)
						if i < len(poly)-1:
							poly_str += " "
					poly_str += "\n"
					file.write(poly_str) 
				...
				#print str([round(loc[0][0] ,3), round(loc[0][1] ,3)]) + ","
				curr_poly.append([round(loc[0][0] ,3), round(loc[0][1],3)])
				#pygame.draw.circle(screen, (255,255,0), mouseClick, 7)
				pygame.draw.circle(screen, (0,0,255), mouseClick, 4)
				#label = myfont.render(str(loc), 3, (0,0,0))
				#label_loc = (mouseClick[0] -30, mouseClick[1] - 20)
				#screen.blit(label,label_loc)
		else:
			pygame.draw.rect(screen, green,(90,450,100,50))
